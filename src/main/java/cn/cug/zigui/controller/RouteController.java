...
public class RouteController {
    ...
    @GetMapping("/wx")
    @SaCheckLogin
    public Result<?> listForWx() {
        List<VUniRoute> result = CollUtil.list(Boolean.TRUE);
        List<Route> list = iRouteService.list(Wrappers.<Route>lambdaQuery().eq(Route::getSaved, Boolean.TRUE).orderByAsc(Route::getRouteName));
        list.forEach(route -> {
            VUniRoute vUniRoute = new VUniRoute();
            BeanUtil.copyProperties(route, vUniRoute);
            ...
            List<RoutePoint> points = iRoutePointService.list(Wrappers.<RoutePoint>lambdaQuery().eq(RoutePoint::getRouteId, route.getRouteId()).orderByAsc(RoutePoint::getPointName));
